...
bool backtracking(int cnt, pi red, pi blue, vector<vector<char>> board) {
    ...
    for(int i = 0; i < 4; i++) { // i : 방향
        ...
        if(red_first) {
            ...
            if(!r_escape && !b_escape) { // 1-2. 빨간 구슬, 파란 구슬 새 위치로 이동
                bool tmp = backtracking(cnt+1, tmp_red, tmp_blue, tmp_board);
                if(tmp) {
                    return true;
                }
            }
...
int main() {
    ...
    cout << backtracking(0, red, blue, board);
    return 0;
}