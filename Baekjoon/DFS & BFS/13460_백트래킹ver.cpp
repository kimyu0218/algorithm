...
bool canEscape(char color, int dir, pi &bead, vector<vector<char>> &board) {
    ...
    board[er][ec] = color; // 구슬이 멈추는 위치
    bead = {er, ec};
    return false;
}

void backtracking(int cnt, pi red, pi blue, vector<vector<char>> board) {
    if(cnt == CNT) { // 탈출 불가
        return;
    }
    ...
    for(int i = 0; i < 4; i++) { // i : 방향
        ...
        if(red_first) {
            ...
            if(!r_escape && !b_escape) { // 1-2. 빨간 구슬, 파란 구슬 새 위치로 이동
                backtracking(cnt+1, tmp_red, tmp_blue, tmp_board);
            }
        }
            // 2. 파란 구슬이 먼저 움직이는 경우
        else {
