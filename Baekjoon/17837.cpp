...
void removeFromOriginalPos(int row, int col, int stop) {
    vector<int> tmp;
    for(int i = 0; i < board[row][col].chessman.size(); i++) {
        int num = board[row][col].chessman[i];
        if(num == stop) { // stop 위의 말들은 새로운 칸으로 이동
            if(i != 0) { // 연결 끊기
                int top = board[row][col].chessman[i-1];
                chessman[top].next = -1;
            }
            break;
        }
...
void moveToWhite(int r, int c, int nr, int nc, int next_cnt, vector<int> &moves) {
    removeFromOriginalPos(r, c, moves[0]);
...
int chessGame(int n, int k) {
    ...
    while(++turn <= TURN) { // 턴이 1000을 넘지 않는 경우
        for(int a = 0; a < k; a++) { // a번 말의 이동
            ...
            int d = chessman[a].dir, next = chessman[a].next;

            vector<int> moves = {a}; // a번 말과 함께 이동하는 말들
            while(next != -1) {
                moves.push_back(next);
                next = chessman[next].next;
            }
